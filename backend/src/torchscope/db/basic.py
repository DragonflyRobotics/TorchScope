...
def new_run_session(db_conn, model_id, columns: dict, run_name=None): 
    ...
    existing_runs = [row[0] for row in db_conn.execute(f""" SELECT DISTINCT runs FROM model_runs WHERE model = '{model_id}' """).fetchall()]
...
def new_model_session(db_conn, project_name, model_name=None):
    ...
    existing_models = [row[0] for row in db_conn.execute(f""" SELECT DISTINCT model FROM project_models WHERE project = '{project_name}' """).fetchall()]
